...
public abstract class UrlDataExtractor {
    ...
    public static int getIndex(final String url) {
        ...
        return UrlDataExtractor.getIndex(url, -1);
    }
    ...
    public static int getIndex(final String url, final int fallback) {
        ...
        final var matcher = UrlDataExtractor.URL_PATTERN.matcher(url);
        if (!matcher.find()) {
            return fallback;
        }
        final var idx = matcher.group("idx");
        if (idx == null) {
            return fallback;
        }
    ...
    public static String getEncodedDiagram(final String url) {
        ...
        return UrlDataExtractor.getEncodedDiagram(url, null);
    }
    ...
    public static String getEncodedDiagram(final String url, final String fallback) {
        ...
        final var encoded = matcher.group("encoded");
        if (encoded == null) {
            return fallback;
        }
        return encoded;
