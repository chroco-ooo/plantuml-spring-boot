...
import jp.livlog.plantuml.servlet.utility.Configuration;
...
public class PlantUmlServlet extends AsciiCoderServlet {
    ...
    public static String stringToHTMLString(final String string) {
        ...
        var offset = 0;
        while (offset < length) {
            final var c = string.codePointAt(offset);
            switch (c) {
                case ' ':
                    sb.append(' ');
                    break;
    ...
    public void doGet(final HttpServletRequest request, final HttpServletResponse response) throws IOException, ServletException {
        ...
        final var text = this.getText(request).trim();

        // no Text form has been submitted
        if (text.isEmpty()) {
            this.redirectNow(request, response, PlantUmlServlet.DEFAULT_ENCODED_TEXT);
            return;
        }

        // diagram index to render
        final var idx = UrlDataExtractor.getIndex(request.getRequestURI());

        // forward to index.jsp
        final String path;
        final var view = request.getParameter("view");
        if (view != null && "previewer".equalsIgnoreCase(view)) {
            path = "/previewer.jsp";
        } else {
            path = "/index.jsp";
        }
    ...
    protected void doPost(
        ...
        try {
            ...
            encoded = PlantUmlServlet.DEFAULT_ENCODED_TEXT;
            e.printStackTrace();
        }

        this.redirectNow(request, response, encoded, idx);
    }
    ...
    private String getText(final HttpServletRequest request) throws IOException {
        ...
        // 2. metadata
        final var metadata = request.getParameter("metadata");
        if (metadata != null) {
            try (var img = PlantUmlServlet.getImage(new URL(metadata))) {
                final var metadataTag = new MetadataTag(img, "plantuml");
                final var data = metadataTag.getData();
                if (data != null) {
                    return data;
                }
            }
     ...
     * @throws IOException if an input or output exception occurred
     */
    private String getTextFromUrl(final HttpServletRequest request) throws IOException {

        return this.getTranscoder().decode(this.getEncodedTextFromUrl(request));
    }
     ...
     * @throws IOException if an input or output exception occurred
     */
    private void prepareRequestForDispatch(final HttpServletRequest request, final String text, final int idx) throws IOException {

        final var encoded = this.getTranscoder().encode(text);
        // diagram sources
    ...
    private static HttpURLConnection getConnection(final URL url) throws IOException {
        ...
        if (url.getProtocol().startsWith("https")) {
            final var con = (HttpsURLConnection) url.openConnection();
     ...
     * @throws IOException if an input or output exception occurred
     */
    private static InputStream getImage(final URL url) throws IOException {

        InputStream is = null;
        final var con = PlantUmlServlet.getConnection(url);
